...
class Kms_Auth_SessionKey
{
    ...
    private function crackSessionKey()
    {
        ...
        if ($decodedString === false)
            return; // could not base64 decode the string, isValid stays false

        $parts = explode(self::SESSION_SIGNATURE_INFO_SEPARATOR, $decodedString);
        if (count($parts) != 2)
            return; // unexpected content in base64 decoded string, isValid stays false

        $originalSignature = $parts[0];
        $this->strInfo = $parts[1];
        $sig = self::createSignature($this->secret, $this->strInfo);
        if ($sig != $originalSignature)
            return; //signature not valid - spoof attempt, isValid stays false
